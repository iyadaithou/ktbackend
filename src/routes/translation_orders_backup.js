...
router.post('/', async (req, res) => {
  try {
    ...
    (async () => {
      const subject = 'Translation order received';
      const html = `<p>Order Code: <strong>${orderCode}</strong></p><p>We received your translation request (${from_language} → ${to_language}) for ${pageCountNum} page(s).</p><p>Total: $${(amount_cents/100).toFixed(2)} ${currency}. Complete payment to proceed.</p>`;
      ...
      if (contact_phone) {
        const smsResult = await sendSms({ to: contact_phone, body: `Pythagoras: Translation order created. Pay to proceed. Amount $${(amount_cents/100).toFixed(2)}.` });
...
router.get('/debug/email-config', async (req, res) => {
  try {
    const config = {
      ...
      allEnvVars: Object.keys(process.env).filter(key => key.includes('BREVO') || key.includes('EMAIL') || key.includes('SENDGRID')),
